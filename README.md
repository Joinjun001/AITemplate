...
- **비용 폭탄을 막으려면**: `ANTHROPIC_API_KEY` 환경변수를 이 스크립트를 실행하는 프로세스/
  ...
  전환됩니다. Extra Usage(종량제 오버플로우)도 켜지 마세요. 두 CLI 모두 `claude login` /
  `gemini` 로그인으로 인증된 상태로만 실행되어야 합니다.
...
OS 스케줄러 (예: 5분마다)
  ...
  tasks/queue.jsonl (todo / in_progress / in_review / done / blocked)
   ...
   ┌────┴─────────────────┐
   ▼                       ▼
worker_gemini.py         worker_claude_impl.py
...
2. `claude` CLI 설치 후 `claude login` (구독 로그인. **API 키 사용 금지**)
3. `gemini` CLI 설치 후 개인 구글 계정으로 로그인
4. `git`
...
- **한도 감지 문구**: `scripts/common.py`의 `detect_limit_and_set_cooldown` 함수(`LIMIT_PATTERN`)가
  ...
  Code나 Gemini CLI가 실제로 내보내는 메시지가 바뀌면 이 정규식을 실제 출력(`state/orchestrator.log`
  에 일부가 남습니다)에 맞게 수정하세요.
...
- Windows에서 npm으로 설치한 CLI(`claude.cmd`, `gemini.cmd` 등)를 못 찾는 경우, PATH에 npm
  글로벌 설치 경로가 등록돼 있는지 확인하세요(`common.py`가 `shutil.which`로 찾습니다).
